...
export class CategoriesComponent extends AppComponentBase {
    @ViewChild('createOrEditCategoryModal', { static: true })
    createOrEditCategoryModal: CreateOrEditCategoryModalComponent;
    ...
    deleteCategory(category: CategoryDto): void {
        this.message.confirm('', this.l('AreYouSure'), (isConfirmed) => {
            if (isConfirmed) {
                this._categoriesServiceProxy.delete(category.id).subscribe(() => {
                    this.reloadPage();
